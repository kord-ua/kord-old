...
class TextSrc
{
     ...
     *
     *     $text = \KORD\Text::limitWords($text);
     *
     ...
     *
     *     $text = \KORD\Text::limitChars($text);
     *
     ...
     * @return  string
     * @uses    \KORD\UTF8::strlen
     * @uses    \KORD\UTF8::substr
     */
     ...
     *
     *     echo \KORD\Text::alternate('one', 'two'); // "one"
     *     echo \KORD\Text::alternate('one', 'two'); // "two"
     *     echo \KORD\Text::alternate('one', 'two'); // "one"
     *
     ...
     *
     *
     *     $str = \KORD\Text::random(); // 8 character random string
     *
     ...
     *
     *      $str = \KORD\Text::ucfirst('content-type'); // returns "Content-Type"
     *
     ...
     *
     *     $str = \KORD\Text::reduceSlashes('foo//bar/baz'); // "foo/bar/baz"
     *
     ...
     *     echo \KORD\Text::censor('What the frick, man!', [
     *         'frick' => '#####',
     *     ]);
     *
    ...
    public static function censor($str, $badwords, $replacement = '#', $replace_partial_words = true)
    {
        ...
        if (UTF8::strlen($replacement) == 1) {
            $regex .= 'e';
            return preg_replace($regex, 'str_repeat($replacement, \KORD\UTF8::strlen(\'$1\'))', $str);
        }
     ...
     *
     *     $match = \KORD\Text::similar(['fred', 'fran', 'free']); // "fr"
     *
     ...
     * will not be altered.
     *
     *     echo \KORD\Text::autoLink($text);
     *
     * [!!] This method is not foolproof since it uses regex to parse HTML.
     *
     * @param   string  $text   text to auto link
     * @return  string
     * @uses    \KORD\Text::autoLinkUrls
     * @uses    \KORD\Text::autoLinkEmails
     */
     ...
     *
     *     echo \KORD\Text::autoLinkUrls($text);
     *
     * [!!] This method is not foolproof since it uses regex to parse HTML.
     *
     * @param   string  $text   text to auto link
     * @return  string
     * @uses    \KORD\HTML::anchor
     */
    public static function autoLinkUrls($text)
    {
        // Find and replace all http/https/ftp/ftps links that are not part of an existing html anchor
        $text = preg_replace_callback('~\b(?<!href="|">)(?:ht|f)tps?://[^<\s]+(?:/|\b)~i', '\KORD\Text::autoLinkUrlsCallback1', $text);

        // Find and replace all naked www.links.com (without http://)
        return preg_replace_callback('~\b(?<!://|">)www(?:\.[a-z0-9][-a-z0-9]*+)+\.[a-z]{2,6}[^<\s]*\b~i', '\KORD\Text::autoLinkUrlsCallback2', $text);
    }
     ...
     * be altered.
     *
     *     echo \KORD\Text::autoLinkEmails($text);
     *
     * [!!] This method is not foolproof since it uses regex to parse HTML.
     *
     * @param   string  $text   text to auto link
     * @return  string
     * @uses    \KORD\HTML::mailto
     */
    public static function autoLinkEmails($text)
    {
        ...
        return preg_replace_callback('~\b(?<!href="mailto:|58;)(?!\.)[-+_a-z0-9.]++(?<!\.)@(?![-.])[-a-z0-9.]+(?<!\.)\.[a-z]{2,6}\b(?!</a>)~i', '\KORD\Text::autoLinkEmailsCallback', $text);
    }
     ...
     *
     *     echo \KORD\Text::bytes(filesize($file));
     *
     ...
     *     echo \KORD\Text::number(1024);
     *
     *     // Display: five million, six hundred and thirty-two
     *     echo \KORD\Text::number(5000632);
     *
     ...
     *     $browser = \KORD\Text::userAgent('browser');
     *
