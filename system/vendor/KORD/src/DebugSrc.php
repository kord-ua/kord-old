...
class DebugSrc
{
     ...
     *     echo \KORD\Debug::vars($foo, $bar, $baz);
     *
    ...
    {
        return Debug::getDump($value, $length, $level_recursion);
    }
     ...
     * @return  string
     */
    protected static function getDump(& $var, $length = 128, $limit = 10, $level = 0)
    {
        if ($var === null) {
            ...
            if (empty($var)) {
                ...
                foreach ($var as $key => & $val) {
                    ...
                    $output[] = "$space$s$key => " . Debug::getDump($val, $length, $limit, $level + 1);
                }
                unset($var[$marker]);
            ...
            if (empty($var)) {
                ...
                foreach ($array as $key => & $val) {
                    ...
                    $output[] = "$space$s$access $key => " . Debug::getDump($val, $length, $limit, $level + 1);
                }
                unset($objects[$hash]);
     ...
     *     echo \KORD\Debug::source(__FILE__, __LINE__);
     *
     ...
     *     echo implode('<br/>', \KORD\Debug::trace());
     *
     * @param   array   $trace
     * @return  string
     */
