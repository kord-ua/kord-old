...
class Code128Src extends AdapterAbstract
{
    ...
    public function hasValidCharacters($value)
    {
        ...
        $read = $set;
        if ($set != '') {
            $value = UTF8::substr($value, 1, null);
        }

        // process barcode
        while ($value != '') {
            $char = UTF8::substr($value, 0, 1);
        ...
        if (($value != '') AND (UTF8::strlen($value) != 1)) {
            return false;
        }
    ...
    protected function code128($value)
    {
        ...
        $value = UTF8::substr($value, 1, null);
        while (UTF8::strpos($value, 'Š') OR ($value != '')) {
            $char = UTF8::substr($value, 0, 1);
            if ($read == 'C') {
                $char = UTF8::substr($value, 0, 2);
            }
            ...
            $value = UTF8::substr($value, 1);
            ++$pos;
            if ((UTF8::strpos($value, 'Š') == 1) AND (UTF8::strlen($value) == 2)) {
        ...
        if ((UTF8::strpos($value, 'Š') != 1) OR (UTF8::strlen($value) != 2)) {
            // return false if checksum is not readable and true if no startvalue is detected
            return (!$usecheck);
        }

        $mod = $sum % 103;
        if (UTF8::substr($value, 0, 1) == $this->chr128($mod, $set)) {
            return true;
        }
