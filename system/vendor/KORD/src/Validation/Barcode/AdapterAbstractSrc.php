...
abstract class AdapterAbstractSrc extends RuleAbstract implements AdapterInterface
{
    ...
    protected $options = [
        ...
        'use_checksum' => true, // Is a checksum value included in a barcode?, boolean
    ];
    
    /**
     * Sets the allowed length of this barcode
     *
     ...
     * @return $this
     */
    protected function setUseChecksum($check)
    {
        $this->options['use_checksum'] = (bool) $check;
        return $this;
    }
    ...
    {
        if (!is_string($value)) {
            $this->addError('barcodeInvalid');
            return false;
        }

        if (!$this->hasValidLength($value)) {
            $length = $this->getLength();
            $this->addError('barcodeInvalidLength', [
        ...
        if (!$this->hasValidCharacters($value)) {
            $this->addError('barcodeInvalidChars');
            return false;
        }

        if ($this->getUseChecksum() AND !$this->hasValidChecksum($value)) {
            $this->addError('barcodeFailed');
            return false;
        }
    ...
    public function hasValidChecksum($value)
    {
        ...
        if (!empty($checksum) AND method_exists($this, $checksum)) {
