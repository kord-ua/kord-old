    ...
    for ($i = 0; $i < $len; $i++) {
        ...
        if ($m_state == 0) {
            ...
            if (0 == (0x80 & $in)) {
                ...
                trigger_error('\KORD\UTF8::toUnicode: Illegal sequence identifier in UTF-8 at byte ' . $i, E_USER_WARNING);
                return false;
            }
        } else {
            ...
            if (0x80 == (0xC0 & $in)) {
                ...
                if (0 == --$m_state) {
                    ...
                    if (((2 == $m_bytes) AND ( $m_ucs4 < 0x0080)) OR ( (3 == $m_bytes) AND ( $m_ucs4 < 0x0800)) OR ( (4 == $m_bytes) AND ( $m_ucs4 < 0x10000)) OR ( 4 < $m_bytes) OR
                            ...
                            ( $m_ucs4 > 0x10FFFF)) {
                        trigger_error('\KORD\UTF8::toUnicode: Illegal sequence or codepoint in UTF-8 at byte ' . $i, E_USER_WARNING);
                        return false;
                    }
                ...
                throw new \KORD\UTF8\Exception("\KORD\UTF8::toUnicode: Incomplete multi-octet sequence in UTF-8 at byte '{byte}'", [
                    'byte' => $i,
                ]);
            }
        }
