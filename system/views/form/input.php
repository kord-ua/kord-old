...
<?php if ($element->isMultilingual()): ?>
    <?php foreach ($element->getLanguages() as $language):  ?>
        <?php echo $language ?> <input type="<?php echo $element->getType() ?>" name="<?php echo $element->getName() ?>[<?php echo $language ?>]" value="<?php echo \KORD\Arr::get($element->getValue(),$language) ?>"/><br/>
    <?php endforeach ?>
<?php else: ?>
