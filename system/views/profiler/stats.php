...
<style type="text/css">
<?php include \KORD\Core::findFile('views', 'profiler/style', 'css') ?>
</style>

<?php
$group_stats = \KORD\Profiler::groupStats();
$group_cols = ['min', 'max', 'average', 'total'];
$application_cols = ['min', 'max', 'average', 'current'];
?>

<div class="kord">
    <?php foreach (\KORD\Profiler::groups() as $group => $benchmarks): ?>
        <table class="profiler">
            ...
            <?php foreach ($benchmarks as $name => $tokens): ?>
                <tr class="mark time">
                    <?php $stats = \KORD\Profiler::stats($tokens) ?>
                    <th class="name" rowspan="2" scope="rowgroup"><?php echo \KORD\HTML::chars($name), ' (', count($tokens), ')' ?></th>
                    <?php foreach ($group_cols as $key): ?>
    ...
    <table class="profiler">
        <?php $stats = \KORD\Profiler::application() ?>
